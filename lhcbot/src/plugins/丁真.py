...
def 丁真tts(text: str):
    ...
    response = requests.get('https://api.lolimi.cn/API/yyhc/dz.php', params=params, headers=headers)
...
# 定义命令处理器
abstract = on_command("丁真说", priority=3, block=True)

# 处理用户输入的命令和参数
@abstract.handle()
async def handle_input(state: T_State, arg: Message = CommandArg()):
    # 提取命令后紧跟的文字作为待转换的内容
    state["textdingzhen"] = arg.extract_plain_text().strip()

# 当“text”参数获取到时执行后续逻辑
@abstract.got("textdingzhen", prompt=f"你想让ta说什么？")
async def generate_tts(bot: Bot, event: Event, text: str = ArgStr("textdingzhen")):
    try:
        # 调用TTS API 并记录日志
        url = f'https://api.lolimi.cn/API/yyhc/dz.php?msg={text}'
        logger.info(url)

        # 将获取到的音频内容以消息段的形式发送出去（假设该API返回的是可以直接播放的语音文件）
