...
from nonebot import get_driver, on_request, on_notice, on_command, on_message, on_fullmatch
...
#             MessageSegment.text('这是哪里？哦？让我康康！') + MessageSegment.face(269)))
#     else:
#         await bot.send_group_msg(group_id=event.group_id, message=Message(
...
sendlike = on_fullmatch('赞我', block=True, priority=96)

@sendlike.handle()
async def sendlike_handle(bot: Bot, event: Event):
    usrid = int(event.get_user_id())
    await bot.send_like(user_id=usrid, times=10)
    await sendlike.send("已经点满了,没成功的话加好友试试")
